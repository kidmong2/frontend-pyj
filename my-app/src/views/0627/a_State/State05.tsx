...
export default function State05() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // false || true => true
    // false && true => X (오류 발생하지 않음)
    if(!email || !validateEmail(email)) {
      tempErrors.email = '유효한 이메일을 입력하거나 이메일을 입력해주세요'
      isValid = false;
    }
    ...
    <div style={{ margin: '20px', padding: '10px'}}>
      ...
      <form onSubmit={handleSubmit}>
        <div>
          ...
          />
          {/* 
            상태 관리되는 errors에 id 속성이 존재한다면: 아이디에 대한 에러 메시지 출력 
          */}
          {/* 삼항 연산자 */}
          {/* and 연산자 */}
          {/* >> 출력되는 메시지(p태그) 글자색 red */}
        ...
        <div>
          ...
          />
          {/* 
            상태 관리되는 errors에 password 속성이 존재한다면: 비밀번호에 대한 에러 메시지 출력 
          */}
        ...
        <div>
          ...
          />
          {/* 
            상태 관리되는 errors에 email 속성이 존재한다면: 이메일에 대한 에러 메시지 출력 
          */}
