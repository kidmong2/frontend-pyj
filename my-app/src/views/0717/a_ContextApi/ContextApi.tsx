...
interface UserContextType {
  user: User | null;
  // useState를 useReducer로 인식한 상태 설정 함수타입
...
const Navbar = () => {
  ...
  return <div>반갑습니다, {user ? user.name : "Guest"}님 </div>;
}
...
const Profile = () => {
  ...
  if (!user) {
    return <button onClick={() => setUser({name: '박용재'})}>로그인</button>
  }
  
  return (
    <div>
      <h2>{user.name}</h2>
      <button onClick={() => setUser({name: '테스트'})}>로그아웃</button>
    </div>
  )
}
