...
- DiaryEntry 컴포넌트
  : Props로 date(string), content, mood(Mood) 받음
    >> 일기의 날짜, 내용, 기분을 보여주는 컴포넌트 (h2, p, p)

- DiaryList 컴포넌트
  : 여러 개의 DiaryEntry 컴포넌트를 렌더링
  : 전체 일기 목록(entries - DiaryEntryProps)과 현재 필터링 된 기분 상태를 props로 받음
...
//& DiaryEntry
const DiaryEntry: React.FC <DiaryEntryProps>  = ({date, content, mood}) => {
  return (
    <div>
      <h2>오늘의 날짜: {date}</h2>
      <p>일기: {content}</p>
...
// const FilteredAndMap = entries
// .filter((Mood) => Mood.mood === '좋음' || '나쁨' || '보통')
// .map((entries, index) => (
//   <h2 key={index}>
//     {entries.date}
//   </h2>
