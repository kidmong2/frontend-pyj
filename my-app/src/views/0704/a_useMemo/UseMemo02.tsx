...
export default function UseMemo02() {
  ...
  // const filteredList = largeList.filter(item => item.toLowerCase().includes(filter.toLowerCase()));

  //? 2. useMemo를 사용하여 리스트 필터링 로직을 메모이제이션
  // : 필터링 로직은 비용이 큰 작업이므로, filter 상태가 변경될 때만 다시 계산
  const filteredList = useMemo(()=> {
    console.log('필터링 중 입니다.');
  ...
  return (
    <div>
      <input type="text" value={filter} onChange={(e)=> setFilter(e.target.value)} placeholder='Filter List'/>
      ...
      <p>{count}</p>
      <button onClick={() => setCount((prevCount) => prevCount + 1)}>증가버튼</button>
    </div>
  );
}
