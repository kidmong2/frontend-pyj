
{
  ...
  // 4. editTodo: Todo 항목의 task를 편집 - 전체 배열 / 수정하고자 하는 요소의 id / 새롭게 작성될 task 내용
  ...
  // 8. toggleTodo: Todo 항목의 completed 상태를 토글 - 전체배열 / 토글할 항목의 id 
  ...
  function editTodo(todos: ITodoItem[], id: number, newTask: string) {
    return todos.map 
  }
  
  function clearCompleted(todos: ITodoItem[], id: number): ITodoItem[] {
    return todos.filter(completeTodo => completeTodo.id !== id)
  }
