// a_generic01.ts
{
  ...
  // let stringOutput = generic('안녕하세요'); - 오류는 나지 않음
  // let stringOutput = generic<string>(500); - Error


  let numberOutput = generic<number>(500);
  // let numberOutput = generic(500); - 오류는 나지 않음
  ...
  let pariOutput = pair('안녕', 5);
  // let pariOutput2 = pair<string, number>(5, '안녕'); -  Error

  console.log(pariOutput);


  //? 5. 제네릭을 사용하는 컴포넌트들 예시
  //# 1) 제네릭 함수
  function genericFunc<T>(args: T[]): T[] { // string[], num[]
  ...
  console.log(myGenericString.add('박', '용재')); 
  // 박, 용재
