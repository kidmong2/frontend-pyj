//! 1. 비동기 처리 프로그래밍 - 콜백 함수
{
  ...
  */
  ...
  function fetchUserData(userId: number) {
    fetch(`https://jsonplaceholder.typicode.com/users/${userId}`)
      ...
      .then((user) => {
        ...
        //! 두번째 요청: 해당 사용자의 게시물 가져오기
        fetch(`https://jsonplaceholder.typicode.com/posts?userId=${userId}`)
          .then((response) => response.json())
          .then((posts) => {
            console.log(`사용자의 게시물: ${posts}`);
            
            //! 세번째 요청: 해당 사용자의 앨범 가져오기
  ...
  function handleUserData(data: void) {
...
function callback(arg0: { user: any; posts: any; albums: any; }) {
