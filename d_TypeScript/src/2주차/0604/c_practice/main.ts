{
  ...
  */
  ...
  const createUserData = (user: User): HTMLElement => {
    const userDataDiv = document.getElementById('userData') as HTMLElement;
    // madeuserData.className = 'user-data';
  ...
  const displayUsers = (users: Users) => {
    ...
    if (userDatadiv) {
      userDatadiv.innerHTML = '';
      users.forEach((user) => {
        const madeuserData = createUserData(user);
        madeuserData.appendChild(userDatadiv);
      })
    }
  }
  ...
  const addEventListener = () => {
    ...
    if(userIdElement) {
      userIdElement.addEventListener('input', () => {
        const idElement = userIdElement.value;
  ...
  const filterUsers = (users: Users): Users => {
    return users.filter((user) => user.id = userIdElement);
  }
