...
function addToCart (id, name, price, quantity) {
  // 배열.findIndex():
  // : 해당 배열에 일치하는 인덱스 있으면 해당 인덱스 반환
  // : 해당 배열에 일치하는 인덱스 없으면 -1 반환
  const index = cart.findIndex(item => item.id === id); // 상품 id가 일치하는 경우 상품의 인덱스 반환
  
  if (index > -1) { // 상품이 이미 장바구니에 있을 경우
    cart[index].quantity += quantity; // 해당 상품 갯수 업데이트
  } 
...
function updateQuantity(id, quantity) {
  const index = cart.findIndex(item => item.id === id); // 상품의 id가 일치하는 상품의 인덱스 찾기
  ...
  else { // 수량이 유효하지 않거나 상품이 존재하지 않을 경우
...
function displayCart() {
  ...
  cart.forEach(item => {
    console.log(`${item.name} - 가격: $${item.price}, 수량: ${item.quantity}`);
  });
}
