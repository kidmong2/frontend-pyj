...
// - id: 각 할 일의 고유 식별자. 일반적으로 숫자나 문자열로 표현, 배열의 인덱스 + 1 (1부터 1씩 증가하는 정수값)
...
function addTodo(content) {
  const todo = {
    id: todos.length + 1, // 새 할 일에 고유 id 할당 (배열의 길이+1)
    content: content,
    completed: false, // 할 일의 완료 상태를 기본적으로 미완료로 지정
  };
  todos.push(todo); // 생성한 할 일 객체를 todos 배열에 추가

  //? 새로운 할 일이 추가될 때마다 할 일 목록을 콘솔에 출력
  displayTodos();
}
...
function toggleTodoComplete(id) {
  ...
  return todo.id === id ? {...todo, completed: !todo.completed} : todo; //! 이해필요 2 !!!!!
  });
}
...
function displayTodos() {
  ...
  todos.forEach(todo => {
    console.log(`${todo.id}: ${todo.content} - ${todo.completed ? '완료됨' : '미완료'}`);
  });
}

// 함수 사용 예시
console.log('=== 할 일 추가 ===');
addTodo('자바스크립트 공부하기'); // id 1
addTodo('HTML CSS 복습하기'); // id 2
addTodo('정보처리기사 필기 공부하기'); // id 3
