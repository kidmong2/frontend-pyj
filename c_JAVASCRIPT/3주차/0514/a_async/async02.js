...
function loginUser(email, password, callback) {
  ...
  if (user) {
    ...
    // 사용자 역할 기반으로 특정 적근 권한 확인
    // 에러가 존재 하지 않기 때문에 null 반환
...
function checkAccess(roles, callback) {
  ...
  if (accessGranted) {
    callback(null, true); // 접근 권환 있음
  } else {
    callback(null, false); // 접근 권환 없음
  }
}
...
loginUser("이메일1", "비밀번호1", function (error, user) {
  ...
  checkAccess(user.roles, function (error, access) {
    ...
    if (access) {
      return console.log("접근 가능한 관리자입니다.");
    } else {
