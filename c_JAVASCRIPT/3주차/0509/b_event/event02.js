...
function bgChange(e) {
  ...
  // JS에서 스타일 지정 방식 : lowerCamelCase 사용
  e.target.style.backgroundColor = randomColor;
}

// 문서에서 id를 사용해 요소 선택
// : querySelector와 차이점은 '#'을 사용하지 않는다
...
// - 반복문을 사용하여 각 tile 요소에 onclick 이벤트 핸들러 할당
// : 해당 이벤트는 이벤트 객체를 전달 받기
...
form.onsubmit = function (e) {
  ...
  // 참조된 요소의 값을 가져올 때는 참조값.value 속성 사용
  if (fname.value === "" || email.value === "") {
    // >> input 창의 값은 string 이기 떄문에 비워질 경우 빈 문자열로 반환
    e.preventDefault();
    para.textContent = "이름과 이메일 모두 입력해야 합니다!";
  }
};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 캡쳐링의 경우 일반적인 상황에서 실행X
  // : 이벤트 리스너에 세번째 인자로 true 값을 전달할 때만 실행
  ...
  // 자식에서 부모로 진행 (버블링) : true 값을 넣어주면 버블링 실행
});
