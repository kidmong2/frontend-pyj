...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //! 색상 배열 정의 (6개)
  const colors = [
    '#cc3131', '#e3ddbb', '#23b923', '#3939db', '#e5b7e4','#34bcbc'
  ];


  //! 12개의 요소가 담긴 색상 배열 생성
    ...
    for (let i = 0 ; i < 12; i++) {
      ...
      <div class="card">
          <div class="card-inner">
            ...
            <div class="card-back" style="background-color: ${cardColors[i]};">
            </div>
          </div> 
      `;
    }
  ...
  //! 모든 카드 요소에 클릭 이벤트 리스너를 추가하는 함수 (클릭하면 뒤집는 함수)
...
//! 배열의 요소를 무작위로 섞는 함수 제작 
function shuffle(array) {

  for (let i = array.length - 1; i > 0; i--) {
    let j = Math.floor(Math.random() * (i + 1));
    // >> 0부터 i 까지의 무작위 정수를 생성
