...
document.addEventListener('DOMContentLoaded', () => {
  // 요소를 읽어오기
  const header = decument. queryselector('h1)');

  // 텍스트와 ㅅ스타일을 변경 (HTML 속성값을 활용)
  header.textContent = 'HEADER ONE';
  header.style.color = 'white';
  header.stye.backgroundColor = 'black';
...
document.addEventListener('DOMContentLoaded', () => {
  // 요소 읽기
  const items = docmument.queryselector('li');
...
document.addEventListener('DOMContentLoaded', () => {
  let example = documnt.getElementById('example');
